...
COPY --from=BUILD /usr/src/app/target/transitdata-omm-alert-source-jar-with-dependencies.jar /usr/app/transitdata-omm-alert-source.jar

ENTRYPOINT ["java", "-jar", "/usr/app/transitdata-omm-alert-source.jar"]
